...
class GPX: NSObject, NSXMLParserDelegate
{
    ...
    // waypoint is an array of GPX waypoint
    ...
    // the waypoint has lat/long/date but it's also an entry
    ...
    // an entry are hyperlinks associated with a waypoint 
    // so it could be a photo or video you took at that location 
